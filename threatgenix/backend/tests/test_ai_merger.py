...
class TestMergeNewAIThreats:
    ...
    def test_skips_duplicate_ai_threats(self):
        ...
        ai_output = _make_ai_output([
            AIThreatRaw(
                description=f"Data Tampering: Attacker modifies data in payment service",
                stride_category="Tampering",
                severity="High",
                enhances_rule_threat_id=None,
    ...
    def test_does_not_skip_different_stride_category(self):
        ...
        ai_output = _make_ai_output([
            AIThreatRaw(
                description=f"DoS: Attacker floods payment service",
...
class TestMergeEnrichments:
    def test_enrichment_appends_ai_insight(self):
        ...
        assert len(result) == 1
        assert "[AI Insight]" in result[0].description
        assert "PCI context" in result[0].description
        # Original description is preserved at the start
        assert result[0].description.startswith("Rule-based threat T-001")
