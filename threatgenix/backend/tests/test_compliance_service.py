...
async def db_session():
    ...
    async with session_factory() as session:
        for sc, st, cid, cname in SEED_ROWS:
            session.add(ComplianceMappingSQLite(
                stride_category=sc,
                threat_subtype=st,
                nist_control_id=cid,
                nist_control_name=cname,
            ))
...
from app.services.compliance_service import lookup_controls, lookup_controls_batch
...
async def test_lookup_controls_returns_correct_controls(db_session: AsyncSession):
    """Known stride_category + threat_subtype returns matching NIST controls."""
    ...
    ids = {c.nist_control_id for c in controls}
...
async def test_batch_lookup_returns_correct_mapping(db_session: AsyncSession):
    ...
    assert len(result) == 3
    assert {c.nist_control_id for c in result[t1.id]} == {"IA-2", "IA-8"}
    assert {c.nist_control_id for c in result[t2.id]} == {"SC-8", "SC-13"}
    assert {c.nist_control_id for c in result[t3.id]} == {"IA-3"}
