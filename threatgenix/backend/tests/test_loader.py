...
def test_all_20_rules_loaded(rules: list[LoadedRule]) -> None:
    assert len(rules) == 20
...
def test_rule_ids_follow_naming_convention(rules: list[LoadedRule]) -> None:
    """Rule IDs should follow the pattern X-NN where X is a STRIDE letter."""
    import re

    for rule in rules:
        assert re.match(r"^[STRIDE]-\d{2}$", rule.rule_id), (
